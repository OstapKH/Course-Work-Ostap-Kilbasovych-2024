...
// Tumor simulation parameters type definition
	struct TumorParameters {
		...
		TumorParameters(double tumorInitRadius=0.25, double a=0.58, double b=0.30, double p0=0.288, double gamma=0.05, double xDegradation=0.7, int muMotility=3):tumorInitRadius(tumorInitRadius),a(a),b(b),p0(p0),gamma(gamma),xDegradation(xDegradation),muMotility(muMotility){}
	};
...
// Constructors
	Automata(unsigned long int maxCells = 10000, int randSeed=1, double height=100.0, double width=100.0, double deep=100.0, double cellRadius=0.1, TumorParameters tumorParam={0.25, 0.58}, const std::vector<Cell> &cells={});
...
inline double Automata::euclideanDistance(const Point& p1, const Point& p2){

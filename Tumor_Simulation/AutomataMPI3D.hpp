#ifndef AutomataMPI3D_HPP_
#define AutomataMPI3D_HPP_
...
#define D 3.0	// Euclidean spatial dimension (2 dimensions)

// Should not exist more than one automaton, due to the srand() function, which is used during the construction of the Automata object
...
// Constructors
	AutomataMPI3D(int size, int rank, unsigned long int maxCells = 10000, int randSeed=1, double height=5.0, double width=5.0, double deep=5.0, double cellRadius=0.0075, TumorParameters tumorParam={0.15, 0.58});
...
private:
// Private methods
	void serialize(std::vector<std::vector<int>>& neighbors, int* buffer);
	void deserialize(std::vector<int>& sizesOfEachVector, int* buffer);
...
// Actually works for 2D points only
inline double AutomataMPI3D::euclideanDistance(const Point& p1, const Point& p2){
	return sqrt(pow(p1.x - p2.x, 2) + pow(p1.y - p2.y, 2) + pow(p1.z - p2.z, 2));
}

#endif /* AutomataMPI3D_HPP_ */
